...
| `poll_momentum` | 59.6% train / 39.4% at 2026 serve (was 54.1% / **0.0%**) | Least-squares slope of **all** the candidate's dated polls — rising or falling. NaN with fewer than 3 dated polls (never a silent 0, which would read as "flat"). **Changed 2026-08-03**: was the slope over the final 60 days only, which is 0.0% populated at serve time for a general election (a fixed date means nothing is within 60 days of it until ~September — measured: min `days_to_elec` 95, 0 of 1,667 feed rows qualified) while still being a production feature ranked 12th of 187 by gain and appearing in 82 dashboard SHAP blocks with a null value. The two definitions correlate r=0.917 where both exist, so the window was discarding data rather than isolating a distinct signal. Accuracy impact is noise-level across seeds — the change is justified by serve-time availability. **Computed by the single shared `F.poll_momentum_slope()`** — `features_primary.py` carried a forked 60-day copy until 2026-08-03, which silently kept both primary models on the old definition through a full retrain; do not re-implement it. Coverage figures here are the GENERAL model's; on the primary side it is 29.1% → 53.1%, and there the old/new correlation is only r=0.353 (not 0.917), so for primaries this is a materially different feature rather than a near-duplicate. |
...
| `is_incumbent` | 3.3% | ⚠️ **Party-level, not personal — the name is a misnomer.** 1 if this candidate's **party** holds the seat; NaN (not 0) when the seat's incumbent party is unknown. It is computed as `incumbent_party == candidate party` and nothing more: `races.csv` carries `incumbent_party` but **no incumbent name**, so per-person incumbency does not exist anywhere in the inputs. Consequence: *every* candidate of the holding party reads 1, so a race can show several "incumbents" (16 of 114 in the 2026 general feed; AK-Gov shows 3, SC-Sen 7). Do not read it as "this person holds the seat." See CONCERNS.md #19. |
